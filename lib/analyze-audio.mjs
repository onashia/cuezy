 ...
 *
 * Cancellation is best-effort: it is checked before and after synchronous
 * ffmpeg/ffprobe/yt-dlp calls, between segments, and during retry sleeps. A
 * Shazam request already in flight may continue until the shazam-api package
 * returns because that dependency does not expose AbortSignal support.
 *
...
export async function analyzeAudio(inputPathOrUrl, options = {}, callbacks = {}) {
  ...
  try {
    ...
    if (isURL) {
      ...
      file = downloadURL(input, opts.outputDir, {
        signal,
