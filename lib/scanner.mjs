...
export async function scan(file, opts = {}) {
  ...
  throwIfAborted(signal);
  const duration = opts.duration ?? getDuration(file);
  const totalSegs = Math.max(0, Math.floor((duration - segment) / step) + 1);
  ...
  for (let pos = start; pos + segment <= duration; pos += step) {
    ...
    if (!extractSegment(file, pos, segment, segPath, { signal })) {
      const segmentResult = {
        status: 'skipped',
        file,
        positionSec: pos,
