...
export function buildMarkdownExport(rows, meta = {}) {
  const lines = normalizeTrackRows(rows, meta)
    ...
    .map(row => joinPresent([row.timestamp, row.artist, row.title], ' - '));
...
export function buildTxtExport(rows, meta = {}) {
  return normalizeTrackRows(rows, meta)
    .filter(row => row.timestamp || row.artist || row.title)
    .map(row => joinPresent([
      joinPresent([row.artist, row.title], ' - '),
      row.timestamp,
    ], ' '))
    .join('\n') + '\n';
}
...
function cueFileType(audioFilename) {
  return /\.(wav|wave)$/i.test(String(audioFilename || '')) ? 'WAVE' : 'MP3';
}
...
export function buildCueExport(rows, meta = {}) {
  ...
  let cue = `REM GENRE DJ Mix\nTITLE "${title}"\nFILE "${cueText(audioFilename)}" ${cueFileType(audioFilename)}\n`;

  let trackNumber = 0;
  normalizeTrackRows(rows, meta).forEach(track => {
